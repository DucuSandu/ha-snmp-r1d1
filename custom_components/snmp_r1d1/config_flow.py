...
class SnmpFlowHelper:
    ...
    async def handle_test(flow, user_input=None):
        ...
        client = SnmpClient(flow._data[CONF_DEVICE_IP], credentials)
...
class SnmpOptionsFlow(config_entries.OptionsFlow):
    ...
    # async def async_step_init(self, user_input=None):
    #     """Handle the initial step when clicking 'Configure'."""
    #     _LOGGER.info("# Entering step init")
    #     errors = {}
    #     if user_input is not None:
    ...
    #             _LOGGER.error("Invalid action selected: %s", action)

    #     device_name = self._device_name
    #     options = [
    #         {"value": "reconfigure", "label": f"Reconfigure {device_name}"},
    #         {"value": "delete", "label": f"Delete {device_name}"},
    #     ]
    #     return self.async_show_form(
    #         step_id="init",
    #         data_schema=vol.Schema({
    ...
    #         }),
    #         errors=errors,
    #         description_placeholders={"title": f"Manage {device_name}"}
    #     )

    # async def async_step_menu(self, user_input=None):
    #     """Handle the initial menu step."""
    #     _LOGGER.info("# Entering step menu")
    #     errors = {}
    #     if user_input is not None:
    #         _LOGGER.debug("        Step menu received user_input: %s", user_input)
    #         if not isinstance(user_input, dict) or "action" not in user_input:
    #             errors["base"] = "action_required"
    #             _LOGGER.error("        Step menu invalid or missing action in user_input: %s", user_input)
    #         else:
    #             action = user_input["action"]
    #             _LOGGER.info("        Step menu selected action: %s", action)
    #             if action == "configure":
    #                 _LOGGER.info("        Step menu user selected to configure new device")
    #                 return self.async_abort(reason="start_new_config")
    #             self._mode = action
    #             _LOGGER.debug("        Step menu transitioning to select_device")
    #             return await self.async_step_select_device()
        
    #     _LOGGER.debug("        Step menu rendering form")
    #     return self.async_show_form(
    #         step_id="menu",
    #         data_schema=vol.Schema({
    #             vol.Required("action"): selector.SelectSelector(
    ...
    #                     options=[
    #                         {"value": "configure", "label": "Configure new device"},
    #                         {"value": "reconfigure", "label": "Reconfigure existing device"},
    #                         {"value": "delete", "label": "Delete existing device"},
    #                     ],
    ...
    #         }),
    #         errors=errors,
    #         description_placeholders={"title": "Manage SNMP Device"}
    #     )

    # async def async_step_select_device(self, user_input=None):
    #     """Handle device selection step."""
    #     _LOGGER.info("# Entering step select_device")
    #     errors = {}
    #     device_registry = async_get_device_registry(self.hass)
    #     config_entries = self.hass.config_entries.async_entries(DOMAIN)

    #     _LOGGER.debug("        Step select_device found %d config entries: %s", len(config_entries),
    #                   [{CONF_DEVICE_IP: entry.data.get(CONF_DEVICE_IP),
    #                     CONF_ENTITY_PREFIX: entry.data.get(CONF_ENTITY_PREFIX),
    #                     "entry_id": entry.entry_id} for entry in config_entries])

    #     registry_devices = list(device_registry.devices.values())
    #     _LOGGER.debug("        Step select_device found %d device registry entries: %s", len(registry_devices),
    #                   [{"id": d.id, "config_entries": list(getattr(d, 'config_entries', [])),
    #                     "identifiers": list(d.identifiers)} for d in registry_devices])

    #     device_options = []
    #     for entry in config_entries:
    #         _LOGGER.debug("        Step select_device checking config entry: %s", entry.entry_id)
    #         matched = False
    #         for d in registry_devices:
    #             config_entries_set = getattr(d, 'config_entries', set())
    #             if entry.entry_id in config_entries_set:
    #                 _LOGGER.debug("        Step select_device matched by config_entries: device id %s, entry_id %s", d.id, entry.entry_id)
    #                 device_options.append({
    #                     "value": entry.entry_id,
    #                     "label": f"{entry.data[CONF_DEVICE_IP]} - {entry.data[CONF_ENTITY_PREFIX]}"
    #                 })
    #                 matched = True
    #                 break
    #             elif (DOMAIN, entry.data[CONF_DEVICE_IP]) in d.identifiers:
    #                 _LOGGER.debug("        Step select_device matched by identifiers: device id %s, entry_id %s, ip %s",
    #                               d.id, entry.entry_id, entry.data[CONF_DEVICE_IP])
    #                 device_options.append({
    #                     "value": entry.entry_id,
    #                     "label": f"{entry.data[CONF_DEVICE_IP]} - {entry.data[CONF_ENTITY_PREFIX]}"
    #                 })
    ...
    #                 break
    #         if not matched:
    #             _LOGGER.debug("        Step select_device no match found for entry_id: %s", entry.entry_id)

    #     _LOGGER.debug("        Step select_device created %d dropdown options: %s", len(device_options),
    #                   [opt["label"] for opt in device_options])

    #     if not device_options:
    #         _LOGGER.error("        Step select_device no devices available for selection")
    #         errors["base"] = "no_devices"
    #         _LOGGER.debug("        Step select_device rendering menu form")
    #         return self.async_show_form(
    #             step_id="menu",
    #             data_schema=vol.Schema({}),
    #             errors=errors
    ...
    #     if user_input is not None:
    #         self._entry_id = user_input["device"]
    #         _LOGGER.info("        Step select_device selected device with entry_id: %s", self._entry_id)
    #         if self._mode == "reconfigure":
    #             _LOGGER.debug("        Step select_device transitioning to reconfigure")
    #             return await self.async_step_reconfigure()
    #         _LOGGER.debug("        Step select_device transitioning to delete")
    #         return await self.async_step_delete()

    #     _LOGGER.debug("        Step select_device rendering form")
    #     return self.async_show_form(
    ...
    #         }),
    #         errors=errors,
    #         description_placeholders={"title": "Select Device"}
    #     )
